...
?>
<h1 class="text-bold">Reporte de transacciones</h1>
<br><br>
<div class="row">
    <div class="col-sm-4 float-left">
        <label>Estación:</label>
        <select class="form-control select2" id="cboestacionhorario" onchange="ajaxpage('reporte_transaccion.php?estacion=' + Base64.encode(this.value),'contenido'); setTimeout(function() {formatoTabla(true,true,false,false,true,true,true);},500);">
...
<div class="col-sm-12">
    <div class="card card-default card-solid">
        <div class="card-header with-border">
            <h3 class="card-title">Transacciones de administradores</h3>
        </div>
        <div class="card-body">
            <div class="row">
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            ...
                            <select  class="form-control select2" id="cboanio" onchange="
                                    ...
                                    $.ajax({
                                    url: 'reporte_transaccion.php?estacion=<?= base64_encode($estacion); ?>&anio=' + this.value + '&mes=' + _('cbomes').value,
                        ...
                        <div class="col-sm-6">
                            ...
                            <select  class="form-control select2" id="cbomes" onchange="ajaxpage('../common_files/cargando.html' , 'contenido');
                                    $.ajax({
                                    url: 'reporte_transaccion.php?estacion=<?= base64_encode($estacion); ?>&anio=' + _('cboanio').value + '&mes=' + this.value,
                                    success: function (data) {
                                    _('contenido').innerHTML = data;
                                    setTimeout(function() {formatoTabla(true,true,false,true,true,true,true,('csvHtml5,excelHtml5,pdfHtml5,copyHtml5').split(','));},1000);
                                    }
            ...
            <table id="lstentradas" class="table table-bordered table-hover">
                ...
                <tbody>
                <?php
                $sth = $clsBaseDatos->transaccion_reporte($estacion,$anio, str_pad($mes,2,"0",STR_PAD_LEFT));
