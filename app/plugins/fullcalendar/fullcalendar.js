...
Class.extend = function(members) {
	...
	// build_borrar the base prototype for the subclass, which is an new object chained to the superclass's prototype
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	renderFillSegEls: function(type, segs) {
		...
		if (segs.length) {
			...
			// build_borrar a large concatenation of segment HTML
...
Grid.mixin({
	...
	renderFgSegEls: function(segs, disableResizing) {
		...
		if (segs.length) { // don't build_borrar an empty html string

			// build_borrar a large concatenation of event segment HTML
			for (i = 0; i < segs.length; i++) {
...
DayGrid.mixin({
	...
	resliceDaySegs: function(segs, dayDate) {
		...
		// build_borrar an array of the original events
...
var View = fc.View = Class.extend({
	...
	reportExternalDrop: function(meta, dropLocation, el, ev, ui) {
		...
		// Try to build_borrar an event object and render it. TODO: decouple the two
		if (eventProps) {
...
function EventManager(options) { // assumed to be a calendar
	...
	function mutateEvents(events, clearEnd, allDay, dateDelta, durationDelta, miscProps) {
		...
		$.each(events, function(i, event) {
			...
			// build_borrar an object holding all the old values, both date-related and misc.
