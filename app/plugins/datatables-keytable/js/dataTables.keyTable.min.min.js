...
var a=this,c=this.s.dt,e=b(c.table().node()),d=this.s.namespace,m=!1;"static"===e.css("position")&&e.css("position","relative");b(c.table().body()).on("click"+d,"th, td",function(b){if(!1!==a.s.enable){var d=c.cell(this);d.any()&&a._focus(d,null,!1,b)}});b(k).on("keydown"+d,function(b){m||a._key(b)});if(this.c.blurable)b(k).on("mousedown"+d,function(d){b(d.target).parents(".dataTables_filter").length&&a._blur();b(d.target).parents().filter(c.table().container()).length||b(d.target).parents("div.DTE").length||
b(d.target).parents("div.editor-datetime").length||b(d.target).parents().filter(".DTFC_Cloned").length||a._blur()});if(this.c.editor){var q=this.c.editor;q.on("open.keyTableMain",function(b,c){"inline"!==c&&a.s.enable&&(a.enable(!1),q.one("close"+d,function(){a.enable(!0)}))});if(this.c.editOnFocus)c.on("key-focus"+d+" key-refocus"+d,function(b,c,d,e){a._editor(null,e,!0)});c.on("key"+d,function(b,c,d,e,m){a._editor(d,m,!1)});b(c.table().body()).on("dblclick"+d,"th, td",function(b){!1!==a.s.enable&&
...
g;f++)(!0===b&&a.focused(e[f])||!1===b&&!a.focused(e[f]))&&c.push(e[f]);return c});b(k).on("preInit.dt.dtk",function(a,c){if("dt"===a.namespace){a=c.oInit.keys;var e=g.defaults.keys;if(a||e)e=b.extend({},e,a),!1!==a&&new p(c,e)}});return p});
